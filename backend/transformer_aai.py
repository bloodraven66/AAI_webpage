...
class MultiHeadAttn(nn.Module):
    ...
    def _forward(self, inp, attn_mask=None, return_attn=False):
        ...
        #         rel_pos_vec = self.relative_embed2(embed_idx)
        #         rel_score = torch.matmul(attn_prob.permute(1, 0, 2), rel_pos_vec)
        #         attn_vec += rel_score.permute(1, 0, 2)
...
class TransformerLayer(nn.Module):
    def __init__(self, n_head, d_model, d_head, d_inner, kernel_size, dropout,
        ...
        self.dec_attn = MultiHeadAttn(n_head, d_model, d_head, dropout, relative=10, **kwargs)
...
class FFTransformer(nn.Module):
    def __init__(self, n_layer, n_head, d_model, d_head, d_inner, kernel_size,
        ...
        # self.pos_emb = PositionalEmbedding(384)
    ...
    def forward(self, dec_inp, seq_lens=None, conditioning=0, return_attn=False, use_c=False, use_pos=False):
        out = dec_inp
        for idx, layer in enumerate(self.layers):
            out = layer(out, mask=None, return_attn=return_attn)
            if return_attn:
                out, attn = out
        return out
